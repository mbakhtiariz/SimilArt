...
def index():
	...
	feature_data = pd.read_csv(data_path / r"data\features.csv") # pd.read_csv(os.path.join(data_path, 'features.csv'))
	ids = feature_data['id'].to_list()

	# From metadata, extract data with available ids
	data = pd.read_csv(data_path / r"artsight_csvs\metadatas.csv") # pd.read_csv(os.path.join(data_path, r"metadatas.csv"))
...
def test():
	...
	# From metadata, extract data with available ids
	data = pd.read_csv(data_path / r"artsight_csvs\metadatas.csv") # pd.read_csv(os.path.join(data_path, 'metadatas.csv'))
	data = data.set_index('omni_id')
	data = data.loc[ids].to_json(orient='index')
